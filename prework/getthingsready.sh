...
if [ $(kubectl -n trident get tver -o=jsonpath='{.items[0].trident_version}') != "23.10.0" ]; then
    ...
    wget https://github.com/NetApp/trident/releases/download/v23.10.0/trident-installer-23.10.0.tar.gz -P ~/tarballs
    tar -xf ~/tarballs/trident-installer-23.10.0.tar.gz
...
if [ $(kubectl get -n trident pod | grep Running | grep -e '7/7' | wc -l) -ne 1 ]; then
    ...
    echo "##########################"
    wget https://netapp-my.sharepoint.com/:u:/p/yweisser/EdFitt794J5At0Um3zPBd2AB7QBVz_KnXRvP8o3Ksp_biA?download=1 -O ~/tarballs/trident-acp-23.10.0.tar
    #wget https://netapp-my.sharepoint.com/:u:/p/yweisser/EWOCwLViYhVEl_nCGOujtCkBVRnlYavSGCul0b_S5SM7Qw?download=1 -O ~/tarballs/astra-control-center-certs-23.10.0-68.tar.gz
    #tar -vxzf ~/tarballs/astra-control-center-certs-23.10.0-68.tar.gz
    #openssl dgst -sha256 -verify ~/certs/AstraControlCenterDockerImages-public.pub -signature ~/certs/trident-acp-23.10.0.tar.sig ~/trident-acp-23.10.0.tar

    packsize=$(du --apparent-size --block-size=1 ~/tarballs/trident-acp-23.10.0.tar | awk '{ print $1}')
    ...
    podman load --input ~/tarballs/trident-acp-23.10.0.tar
